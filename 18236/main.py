...
#References: Computation of matrix chain products by Hu and Shing
#and CS-TR-81-875, which includes pascal implementation
...
class hushing:
    def __init__(self, arr) -> None:
        ...
        self.n_arcs = 0
        self.arcs = [arc() for _ in range(self.n+10)]
    ...
    def remove_arc(self, cur_node):
        ...
        #print(f"{cur_node} {hm.support()} u: {hm.u} v:{hm.v}")
        self.con[hm.u].pop()
        self.con[hm.v].pop()
    ...
    def dfs(self, node):
        ...
        if not self.child[node]:
            ...
            #print(f"qid[{node}]: {self.qid[node]}")
            cur.den =cur.base
            #print(11111)
            cur.num = self.w[cur.low]*(cur.den+cur.mul-self.mnmul(node))
            #print(f"num: {cur.num} den:{cur.den} low:{cur.low}")
        ...
        while cur_pq and cur_pq[0].support()>=self.w[cur.low]:
            hm = cur_pq[0]
            cur.den+=hm.den
            #print(f"33333 {len(cur_pq)} {cur_pq[0].support()}")
